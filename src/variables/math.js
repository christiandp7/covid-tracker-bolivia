...
export const getLethalityRate = (deaths, cases, asNumber = false) => {
  // Percents
  if (asNumber) {
...
export const getRecoveredRate = (recovered, cases, asNumber = false) => {
  // Percents
  if (asNumber) {
...
// Mortality Rate
export const getMortalityRate = (deaths, population, asNumber = false) => {
  // per 100mil hab
  if (asNumber) {
...
export const getEffectiveLethalityRate = (
  cases,
  deaths,
  active,
  asNumber = false
  ...
  return replaceDecDotByComma(
    roundNumber((deaths * 100) / (cases - active), 2)
  );
};

// Incidence
export const getIncidenceRate = (cases, population, asNumber = false) => {
  // Percents
  if (asNumber) {
    return roundNumber((cases * 100000) / population, 2);
  }
  return replaceDecDotByComma(
    numberWithDots(roundNumber((cases * 100000) / population, 2))
  );
};

// Tests menos Cases
export const getSuspectsAndDiscartedSum = (tests, cases) => {
  // Percents
  return Math.abs(tests - cases);
};

// Return Active
export const getActiveValue = (cases, deaths, recovered) => {
  // Percents
  return cases - (deaths + recovered);
};

// Bolivia Today Sum
export const sumBOStatus = (deps, statusType, status) => {
  // Percents
  let sum = 0;
  for (let key in deps) {
...
export const generateDailyRecords = (valueArray) => {
  // genera los registros Diarios
  ...
  for (let i = valueArray.length - 1; i >= 0; i--) {
    ...
    if (i !== 0) {
      if (valueArray[i] === valueArray[i - 1]) {
        ...
        // Hace la resta y coloca el valor que es
        backArray[i] = valueArray[i] - valueArray[i - 1];
      }
      //console.log(backArray[j])
    } else {
...
export function moveEjeToDays(timeline, status) {
  const newTimeline = timeline.map((countryItem, i) => {
    ...
    let daysDiff = daysDifference(
      firstCaseDate.Brazil,
      firstCaseDate[countryName]
    ); // Obteniendo los días de referencia
